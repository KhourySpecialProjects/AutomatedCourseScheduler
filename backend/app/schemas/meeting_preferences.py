...
class MeetingPreferencesSchema(BaseModel):
    ...
    """Parse a meeting time string into a list of (meetingDays, start_time, end_time) tuples.
    ...
    def normalize_meeting_time(self) -> list[tuple[str, time, time]]:
        ...
        if not segments:
            raise ValueError(
                f"Unrecognized meeting time format: '{self.meetingTime}'")

        return [
            (days, self.parse_time(start_str, start_period), self.parse_time(end_str, end_period))
